...
// Called by the AdminPanel "Generate & Publish" button via POST request.
// Mirrors the cron pipeline exactly — same quality, same author voice,
// same image pipeline, same key rotation.
//
// Env vars required:
...
export default async function handler(req, res) {
  ...
  if (missing.length > 0) {
    return res.status(500).json({
      success: false,
      error: `Missing env vars: ${missing.join(', ')}. Add in Vercel → Settings → Environment Variables.`,
    });
  }

  // ── Groq key rotator (closure — all functions below share these vars) ─────
  const GROQ_KEYS = [
  ...
  // ── groqCall — inner function so it closes over GROQ_KEYS + rotator ───────
  async function groqCall(messages, maxTokens, retries = 5) {
    ...
    while (attempts < maxTotal) {
      ...
      try {
        ...
        if (status === 429 || msg.includes('rate_limit') || msg.includes('Rate limit')) {
          ...
          const waitMs = 62_000 + (attempts * 2_000);
    ...
    console.log(`   ✗ All keys and retries exhausted`);
    return null;
  }
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`MANUAL PIPELINE STARTED — ${timestamp}`);
  console.log(`Categories: ${totalCategories} | Per category: ${ARTICLES_PER_CATEGORY} | Total: ${totalTarget}`);
  console.log(`Groq keys configured: ${GROQ_KEYS.length}`);
  ...
  try {
    for (const [category, config] of Object.entries(NEWS_SOURCES)) {
      ...
      // Pick topics — groqCall now has no extra params, uses closure
      ...
      for (const topic of validTopics) {
        ...
        // Write article — groqCall uses closure, no extra params needed
        const raw = await groqCall([
          {
            ...
            content:
              ...
              `IMAGE QUERY RULES (4 search strings):\n` +
              `- Person → their role not name\n` +
              `- Place → search directly (e.g. "dalal street mumbai", "nse building")\n` +
              `- Finance/stocks → visuals (e.g. "stock chart trading screen", "sensex graph")\n` +
              `- 3–6 words each\n\n` +
        ...
        // Verify content in DB then publish
        if (score >= AUTO_PUBLISH_SCORE && imageCount >= MIN_IMAGES_TO_PUBLISH && parsed.content?.trim().length > 100) {
          ...
          totalDrafts++;
          const reasons = [
            parsed.content?.trim().length <= 100 && 'no content',
            imageCount < MIN_IMAGES_TO_PUBLISH   && `${imageCount} images`,
            score < AUTO_PUBLISH_SCORE            && `score ${score.toFixed(1)} < ${AUTO_PUBLISH_SCORE}`,
          ].filter(Boolean).join(', ');
    ...
    console.log(`\nMANUAL PIPELINE DONE — ${durationSec}s | Saved: ${totalSaved} | Published: ${totalPublished} | Images: ${totalImagesFetched}\n`);
    ...
    return res.status(500).json({ success: false, error: e.message, timestamp });
  }
}

// ─── RSS fetcher (module-level — no closure needed) ───────────────────────────
  ...
  for (const url of feedUrls) {
    try {
      ...
      matches.forEach(m => {
        ...
        if (t && !t.includes('<!') && t.length > 5) headlines.push(t);
      });
